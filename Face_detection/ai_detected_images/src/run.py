...
def is_ai_generated(image_path, threshold=0.50):
        ...
        if isinstance(result, list):
            for item in result:
                if isinstance(item, dict) and 'label' in item and 'score' in item:
                    if item['label'] == 'artificial' and item['score'] > threshold:
                        return True
                else:
                    print("Error: Unexpected item format:", item)
        else:
...
stored_embeddings = load_images_and_extract_embeddings(
    'C:\\Users\\denni\\OneDrive\\Desktop\\new project\\Face_detection\\ai_detected_images\\stored-faces')
...
def check_matched_image(frame_embedding, stored_embeddings, threshold=0.6):
    for stored_embedding in stored_embeddings:
        similarity_percentage = cosine_similarity(
            frame_embedding, stored_embedding)
    ...
    while not match_found:
        ...
        if ret:
            ...
            if frame_embedding is not None:
                match_found, match_percentage = check_matched_image(
                    frame_embedding, stored_embeddings)
                if match_found:
                    break
...
def upload_file():
    if request.method == "POST":
        ...
        if file:
            ...
            upload_folder = 'C:\\Users\\denni\\OneDrive\\Desktop\\new project\\Face_detection\\ai_detected_images\\stored-faces'
